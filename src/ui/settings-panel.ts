...
class SettingsPanel extends Container {
    constructor(events: Events, tooltips: Tooltips, args = {}) {
        ...
        const unifiedSplatsRow = new Container({ class: 'settings-panel-row' });
        const unifiedSplatsLabel = new Label({ class: 'settings-panel-row-label' });
        i18n.bindText(unifiedSplatsLabel, 'panel.settings.unified-splats');
        const unifiedSplatsToggle = new BooleanInput({
            type: 'toggle',
            class: 'settings-panel-row-toggle',
            value: false
        });
        unifiedSplatsRow.append(unifiedSplatsLabel);
        unifiedSplatsRow.append(unifiedSplatsToggle);
        ...
        // incremental PLY autosave journal
        const journalRow = new Container({
            class: 'settings-panel-row'
        });
        const journalStatus = new Label({
            class: 'settings-panel-row-label'
        });
        const journalButton = new Button({
            class: 'settings-panel-row-button'
        });
        i18n.bindText(journalButton, 'panel.settings.autosave.configure');
        journalStatus.text = i18n.t('panel.settings.autosave.disabled');
        journalButton.on('click', () => events.invoke('journal.configure'));
        events.on('journal.status', (status: string) => {
            journalStatus.text = i18n.t(`panel.settings.autosave.${status}`);
        });
        journalRow.append(journalStatus);
        journalRow.append(journalButton);
        ...
        events.on('view.unifiedSplats', (value: boolean) => {
            unifiedSplatsToggle.value = value;
        });

        unifiedSplatsToggle.on('change', (value: boolean) => {
            events.fire('view.setUnifiedSplats', value);
        });
