...
class IndexRanges {
    ...
    /** Restore ranges previously serialized from the public data buffer. */
    static fromData(data: Uint32Array) {
        return new IndexRanges(data);
    }
    ...
    /** Async variant for multi-million-row background work. */
    static async fromPredicateAsync(total: number, pred: (i: number) => boolean, chunkSize = 256 * 1024) {
        const ranges: number[] = [];
        let rangeStart = -1;
        for (let i = 0; i < total; ++i) {
            if (pred(i)) {
        ...
        if (rangeStart !== -1) emit(ranges, rangeStart, total - rangeStart);
        return new IndexRanges(new Uint32Array(ranges));
    }
