...
const registerEditorEvents = (events: Events, editHistory: EditHistory, scene: Scene) => {
    ...
    events.on('camera.reset', () => {
        ...
        const y = Math.cos(initialAzim * Math.PI / 180) * Math.cos(initialElev * Math.PI / 180);
        const z = -Math.sin(initialElev * Math.PI / 180);
    ...
    events.function('view.transparentLocked', () => {
        return transparentLocked;
    });

    events.on('view.setTransparentLocked', (value: boolean) => {
        setTransparentLocked(value);
    });

    events.on('view.toggleLockedTransparency', () => {
        setTransparentLocked(!transparentLocked);
    });
