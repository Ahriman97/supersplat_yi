...
const initFileHandler = (scene: Scene, events: Events, dropTarget: HTMLElement) => {
    ...
    const importSplatModel = async (files: ImportFile[], animationFrame: boolean) => {
        try {
            ...
            if (!animationFrame && lowerMainFilename.endsWith('.ply') && (scene.getElementsByType(ElementType.splat) as Splat[]).length === 1) {
                const file = mainFile.contents;
                await events.invoke('journal.sourceLoaded', model, {
    ...
    events.function('scene.write', async (fileType: FileType, options: SceneExportOptions, stream?: FileSystemWritableFileStream) => {
        ...
        events.fire('journal.setExporting', true);

        let succeeded = false;
        try {
            await events.invoke('journal.flush');
            ...
            events.fire('journal.setExporting', false);
            if (useSpinner) {
